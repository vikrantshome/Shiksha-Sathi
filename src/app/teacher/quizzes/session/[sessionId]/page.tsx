...
export default function TeacherQuizSessionPage() {
  ...
  return (
    <div className="pb-12 space-y-6">
      <header className="rounded-2xl p-6 shadow-elevated1" style={{ background: "var(--color-surface-container-low)", backgroundImage: "linear-gradient(135deg, var(--color-surface-container-low) 0%, var(--color-surface-container) 100%)" }}>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-3">
            <p className="m-0 text-label-sm text-on-surface-variant flex items-center gap-2">
            ...
            <p className="m-0 text-sm text-on-surface-variant flex items-center gap-3 flex-wrap">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md font-semibold text-sm" style={{ background: "var(--color-primary-container)", color: "var(--color-on-primary-container)" }}>
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M3 3h18v18H3V3zm2 2v14h14V5H5zm2 2h10v2H7V7zm0 4h10v2H7v-2zm0 4h7v2H7v-2z"/></svg>
                {state?.sessionCode}
              </span>
              <span className="font-medium">{questionLabel}</span>
              <span className="px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider" style={{ background: isLive ? "var(--color-error-container)" : isReveal ? "var(--color-tertiary-container)" : "var(--color-surface-container)", color: isLive ? "var(--color-error)" : isReveal ? "var(--color-on-tertiary)" : "var(--color-on-surface-variant)" }}>
                {status}
              </span>
            </p>
          ...
          <div className="flex flex-col gap-2">
            <button
              type="button"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all hover:shadow-elevated1 active:scale-95"
      ...
      {error ? (
        <div className="rounded-lg p-4 text-sm flex items-center gap-3" style={{ background: "var(--color-error-container)", color: "var(--color-on-error)" }}>
      ...
      <section className="rounded-2xl p-6 shadow-elevated1" style={{ background: "var(--color-surface-container-lowest)" }}>
        <div className="flex items-center justify-between gap-4 flex-wrap mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all hover:shadow-elevated1 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            >
              {state?.locked ? (
                ...
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0"/></svg>
              )}
          ...
          {state?.currentQuestion ? (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <h2 className="m-0 text-xl font-bold text-on-surface leading-relaxed flex-1">
                  <span className="mr-2 text-on-surface-variant font-extrabold">{state.currentQuestionIndex + 1}.</span>
                  {state.currentQuestion.text}
                </h2>
                {questionType !== "SHORT_ANSWER" && questionType !== "LONG_ANSWER" && (
                  <span className="shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider" style={{ background: "var(--color-surface-container)", color: "var(--color-on-surface-variant)" }}>
                    FAST FINGERS
                  </span>
                )}
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3 relative z-10">
                          <span className="text-lg font-bold" style={{ color: isCorrect && isReveal ? "var(--color-primary)" : "var(--color-on-surface)" }}>
                            {opt}
                          </span>
                          <span className="text-sm font-extrabold tabular-nums" style={{ color: isCorrect && isReveal ? "var(--color-primary)" : "var(--color-on-surface-variant)" }}>
                            {count} ({pct}%)
                          </span>
                        </div>
                        <div className="mt-3 h-2.5 rounded-full overflow-hidden" style={{ background: "var(--color-surface-container)", opacity: 0.5 }}>
                          <motion.div
                            initial={false}
                            animate={{ width: `${pct}%` }}
                        ...
                        {isCorrect && isReveal && (
                          <svg className="absolute top-3 right-3 w-6 h-6" style={{ color: "var(--color-primary)" }} fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {state.currentQuestion.options.map((opt, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3 relative z-10">
                          <div className="flex items-center gap-3">
                            <span className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: isCorrect && isReveal ? "var(--color-primary)" : "var(--color-surface-container)", color: isCorrect && isReveal ? "var(--color-on-primary)" : "var(--color-on-surface-variant)" }}>
                              {optionLabel}
                            </span>
                        ...
                        {isCorrect && isReveal && (
                          <svg className="absolute top-3 right-3 w-5 h-5" style={{ color: "var(--color-primary)" }} fill="currentColor" viewBox="0 0 24 24">
                            <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/>
                          </svg>
                        )}
                      </motion.div>
              ...
              <div className="flex items-center gap-4 pt-2">
                {state.correctAnswer ? (
                  <div className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold" style={{ background: "var(--color-primary-container)", color: "var(--color-on-primary-container)" }}>
                  ...
                  <p className="m-0 text-sm text-on-surface-variant flex items-center gap-2">
                    ...
                    Reveal the correct answer before moving to the next question
                  </p>
                )}
            ...
            >
              <div className="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center" style={{ background: "var(--color-primary-container)" }}>
                <svg className="w-8 h-8" style={{ color: "var(--color-on-primary-container)" }} fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                </svg>
              </div>
              <p className="text-lg font-medium text-on-surface">Session is in lobby</p>
