# PRISM Prompt: Shiksha Sathi Final Coding Agent
...
This is the primary execution prompt for **Shiksha Sathi**.

Use it to:

- complete the remaining MVP work
- keep Jira, Git, branches, PRs, and sprint state aligned
- verify user-facing work in the browser before closing issues
- prevent false `Done` states and delivery drift
...
Use the PRISM framework below and finish the remaining MVP work in **Shiksha Sathi** professionally.
...
1. audit the repository, Jira board, sprint state, and current delivery drift
2. correct obvious Jira inconsistencies before claiming more progress
...
- cosmetic Jira updates without code
...
- Jira drift has already happened in this project
- some subtasks were previously marked `Done` without same-time code evidence
- some parent issues were left out of sync with their subtasks
- sprint naming and sprint placement previously drifted
- release/readiness issues were previously overstated
...
Treat the remaining delivery path as centered on these issues unless Jira audit proves otherwise:
...
If supporting subtasks exist under these items, use them.
...
- `/Users/anuraagpatil/naviksha/Shiksha Sathi/doc/prompts/prism-prompt-jira-strict-delivery-validation-agent.md`
...
2. identify any issues currently marked `Done`, `In Progress`, or `To Do` that are clearly wrong based on code evidence
